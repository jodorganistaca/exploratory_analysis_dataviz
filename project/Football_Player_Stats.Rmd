...
vp3
vp4
vp5(Goals_tot, PresSucc, GCA, Blocks, Err, Recov, SoT., PasTotCmp., TklDri.,Press., DriSucc., Rec., `AerWon%`)
...
bestPlayers <- filter(bestPlayers,players$Player %in% bestPlayersNames)

min_max_norm <- function(x) {
  (x - min(x)) / (max(x) - min(x))
}

bestPlayersNorm <- as.data.frame(lapply(bestPlayers[3:8], min_max_norm))
bestPlayersNorm['Player'] <- bestPlayers$Player
data <- as.data.frame(matrix( sample( 0:20 , 15 , replace=F) , ncol=5))
rownames(data) <- paste("mister", letters[1:6] , sep="-")
radarchart(bestPlayersNorm)
